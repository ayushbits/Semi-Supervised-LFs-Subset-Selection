...
n_classes =6
n_lfs = 68
with open('/home/ayusham/Semi_Supervised_LFs/Data/TREC/d_processed.p', 'rb') as f:
    while 1:
        try:
            o = pickle.load(f)
        except EOFError:
...
objs = []
with open('/home/ayusham/Semi_Supervised_LFs/Data/TREC/U_processed.p', 'rb') as f:
    while 1:
        try:
            o = pickle.load(f)
        except EOFError:
...
objs = []
with open('/home/ayusham/Semi_Supervised_LFs/Data/TREC/validation_processed.p', 'rb') as f:
    while 1:
        try:
            o = pickle.load(f)
        except EOFError:
...
objs1 = []
with open('/home/ayusham/Semi_Supervised_LFs/Data/TREC/test_processed.p', 'rb') as f:
    while 1:
        try:
            o = pickle.load(f)
        except EOFError:
...
for i in range(n_lfs):
    ...
    for j in range(len(y_valid)):
        if y_valid[j] == new_valid[j][i]:
            correct+=1
    prec_lfs.append(correct/len(y_valid))

## End Quality Quides##
a =  torch.tensor(prec_lfs)
print('after ',prec_lfs)
print(np.mean(prec_lfs))
...
for lo in range(0,num_runs):
    ...
    for epoch in range(100):
        ...
        if len(stop_pahle) >30 and len(stop_pahle_gm) >30 and (all(best_score_lr_val >= k for k in stop_pahle) or \
