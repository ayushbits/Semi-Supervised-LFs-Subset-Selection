...
n_classes = 6
n_lfs = 68
with open('Data/TREC/d_processed.p', 'rb') as f:
    while 1:
        try:
            o = pickle.load(f)
        except EOFError:
...
objs = []
with open('Data/TREC/U_processed.p', 'rb') as f:
    while 1:
        try:
            o = pickle.load(f)
        except EOFError:
...
objs1 = []
with open('Data/TREC/test_processed.p', 'rb') as f:
    while 1:
        try:
            o = pickle.load(f)
        except EOFError:
...
for lo in range(0, num_runs):
    ...
    for epoch in range(100):
        ...
        if len(stop_pahle) > 25 and len(stop_pahle_gm) > 25 and (all(best_score_lr_val >= k for k in stop_pahle) or \
